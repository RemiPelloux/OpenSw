...
namespace {
#ifdef OPEN_SW
constexpr ThreadPerformanceMode DefaultThreadPerformanceMode = ThreadPerformanceMode::System;
#else
constexpr ThreadPerformanceMode DefaultThreadPerformanceMode = ThreadPerformanceMode::Hybrid;
#endif

std::atomic<ThreadPerformanceMode> g_thread_performance_mode{DefaultThreadPerformanceMode};
} // Anonymous namespace

bool SetThreadPerformanceMode(ThreadPerformanceMode mode) {
    switch (mode) {
    case ThreadPerformanceMode::System:
    case ThreadPerformanceMode::Hints:
    case ThreadPerformanceMode::Hybrid:
        break;
    default:
        return false;
    }
    const ThreadPerformanceMode previous =
        g_thread_performance_mode.exchange(mode, std::memory_order_relaxed);
    if (previous == mode) {
        return true;
    }
#ifdef __ANDROID__
    if (mode == ThreadPerformanceMode::System) {
        ADPF::Shutdown();
    }
...
ThreadPerformanceMode GetThreadPerformanceMode() {
    return g_thread_performance_mode.load(std::memory_order_relaxed);
}
...
void SetCurrentThreadPriority(ThreadPriority new_priority) {
    ...
    if (GetThreadPerformanceMode() != ThreadPerformanceMode::Hybrid) {
        return;
    }
...
void SetCurrentThreadToPerformanceCores() {
    ...
    const ThreadPerformanceMode mode = GetThreadPerformanceMode();
    if (mode == ThreadPerformanceMode::System) {
    ...
    SetCurrentThreadCoreGroup(mode == ThreadPerformanceMode::Hybrid ? CoreGroup::Performance
                                                                    : CoreGroup::Unrestricted);
#endif
}
...
void SetCurrentThreadToEfficiencyCores() {
    ...
    SetCurrentThreadCoreGroup(mode == ThreadPerformanceMode::Hybrid ? CoreGroup::Efficiency
                                                                    : CoreGroup::Unrestricted);
#endif
}
...
#if defined(__ANDROID__)
    if (GetThreadPerformanceMode() != ThreadPerformanceMode::System) {
...
void RefreshThreadPolicies() {
    ...
    std::erase_if(state.policies, [&state](const ThreadPolicy& policy) {
        ...
        if (!gone) {
            const CoreGroup group = GetThreadPerformanceMode() == ThreadPerformanceMode::System
                                        ? CoreGroup::Unrestricted
                                        : policy.group;
            ApplyCoreGroupLocked(state, policy.tid, group, &gone);
        }
        return gone;
    });
    SnapshotCanaryLocked(state);
